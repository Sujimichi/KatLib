...
namespace KatLib
{
    ...
    public class Dropdown:DryUIBase
    {
        ...
        public void open(Rect anchor, DryUI window, Dictionary<string, string> list, float width, MenuResponse callback){
            anchor_rec = anchor;
        ...
        void OnGUI(){
            ...
            begin_group(container, () =>{
                style_override = GUI.skin.GetStyle("menu.background");
                v_section(menu_width, w => {
                    GUILayout.Space(2);
                    foreach(KeyValuePair<string, string> pair in menu_content){
                        if(GUILayout.Button(pair.Value, "menu.item")){
                            resp(pair.Key);
                            close_menu();
                        }
                    }
